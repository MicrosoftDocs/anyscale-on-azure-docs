...
The portal pre-populates an Azure Container Registry (ACR) name. Anyscale uses ACR for container image builds. ACR configuration is optional. Accept the default name, enter a custom name, or set **ACR mode** to **No ACR** to skip it. If you skip it, you can [configure container image builds](configure-container-image-builds.md) later.
